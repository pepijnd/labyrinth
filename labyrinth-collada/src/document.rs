...
use {
    Animation, AnimationTarget, BindData, BindDataSet, Joint, JointIndex, Skeleton, VertexWeight,
...
impl ColladaDocument {
            ...
            .get_child("library_animations", self.get_ns())
        {
            Some(library_animations) => {
                ...
                animations
                    .filter_map(|a| {
                        Some(Animation {
                            name: a.get_attribute("name", None).unwrap().to_owned(),
                            targets: self.get_animation(a).unwrap_or_default(),
                        })
                    })
                    .collect()
            }
    ...
    ///
    fn get_animation(&self, animation_element: &Element) -> Option<Vec<AnimationTarget>> {
        Some(
            animation_element
                .get_children("animation", self.get_ns())
                .filter_map(|animation_element| {
                    ...
                    Some(AnimationTarget {
                        target: target.trim_end_matches("/transform").to_owned(),
    ...
    fn get_skeleton(&self, root_element: &Element, bind_data: &BindData) -> Option<Skeleton> {
        ...
        Some(Skeleton {
            name: bind_data.skeleton_name.clone(),
            joints: joints,
            ...
            Some(skeletons) => {
                ...
                if let Some(bind_data) = bind_data_opt {
                    ...
                    for vertex_weight in bind_data.vertex_weights.iter() {
                        ...
                        let find_name = format!("{}_{}", &bind_data.skeleton_name, &joint_name);
                            ...
                            .find(|&(_, weight)| *weight == 0.0)
                        {
                            if let Some((joint_index, _)) = skeleton
                                ...
                                .iter()
                                .enumerate()
                                .find(|&(_, j)| &j.name == &find_name)
                            {
                                vertex_joint_weights.joints[next_index] = joint_index as u8;
